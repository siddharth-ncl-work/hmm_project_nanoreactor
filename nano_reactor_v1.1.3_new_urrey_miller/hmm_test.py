...
def analyseCollStep(start_frame_no,end_frame_no):
  ...
  ref_no_of_edges=ref_frame._orig_frame_graph.number_of_edges()
  print ref_frame.frame_graph.number_of_edges(),ref_frame._orig_frame_graph.number_of_edges()
  for frame_no in range(start_frame_no,end_frame_no+1):
    file.seek(getSeekPos(frame_no))
    frame=frame_class(file,frame_no)
...
max_cycle=int(78311/coll_steps+non_coll_steps)
performance=[[],[],[],[]]
...
# 1) % of the time there is increase in the no. of bonds in collision step
# 2) absolute no. of bonds incresed in collsion step
# 3) % of the time HMM has successfully worked(decreased bonds)
# 4) power of HMM
